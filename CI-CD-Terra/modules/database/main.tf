...
resource "aws_db_subnet_group" "std20_db_subnet_group" {
  name = "${var.tag_header}db-subnet-group"
...
resource "aws_db_instance" "std20_mysql_instance" {
  identifier = "${var.tag_header}mysql-instance"
  ...
  tags = {
    Name = "${var.tag_header}mysql-instance"
  }
}
...
resource "aws_iam_role" "std20_rds_proxy_role" {
  name = "${var.tag_header}rds-proxy-role"
...
resource "aws_iam_role_policy" "std20_rds_proxy_policy" {
  name = "${var.tag_header}rds-proxy-policy"
...
resource "aws_db_proxy" "std20_mysql_proxy" {
  name          = "${var.tag_header}mysql-proxy"
  engine_family = "MYSQL"
